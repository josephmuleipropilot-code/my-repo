...
# NOTE: This module is adapted from the original CLI script. It exposes TOPIC_LIBRARY
# and a helper function `solve_equation` that the Flask app can call.
TOPIC_LIBRARY = {
    "Kinematics": [
        EquationDefinition(
...
def solve_equation(topic_name, equation_index, known_values):
    if topic_name not in TOPIC_LIBRARY:
        raise ValueError("Unknown topic")
    equations = TOPIC_LIBRARY[topic_name]
    if not (0 <= equation_index < len(equations)):
        raise ValueError("Unknown equation index")

    equation = equations[equation_index]
    return solve_missing_variable(equation, known_values)
